...
//!
//! 范围与目标：
//! - 统一网络抽象（RPC、连接池、批量、重试）并提供分布式锁子模块的挂载点。
//! - 面向上层：共识复制、服务发现、负载均衡与调度的网络基石。
//!
//! 语义与不变量（草图）：
//! - 连接池容量/健康度边界由配置约束；`cleanup_expired` 不突破 `min_connections` 下限。
//! - 批量请求在 `batch_size` 或 `batch_timeout` 达到时触发，保证响应对齐 `id/batch_id`。
//! - 重试客户端实施指数退避（简化），需确保被调用方法具备幂等性以避免副作用放大。
//!
//! 参考：gRPC/gobrpc 设计、SRE 背压与流控章节、断路器与限流模式。
